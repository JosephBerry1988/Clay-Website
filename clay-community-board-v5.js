(function () {
  ...
  function styleItem(el) {
    ...
    if (width) {
      ...
      if (vwMatch) {
        ...
        el.style.width = 'clamp(' + (vwNum * 9) + 'px, ' + vwNum + 'vw, 9999px)';
      } else {
  ...
  // Ambient auto-drift, up and to the left, while nothing is being
  // dragged or still settling from a throw. Runs through the exact
  // same proxy -> syncTrack pipeline as manual dragging, so it stays
  // in lockstep with the tiling/reveal system for free.
  var driftSpeedX = -6; // px/sec
  var driftSpeedY = -3; // px/sec
  gsap.ticker.add(function () {
    if ((draggable && draggable.isDragging) || throwTween) return;
    var dt = gsap.ticker.deltaRatio(60) / 60;
    var px = gsap.getProperty(proxy, 'x');
    var py = gsap.getProperty(proxy, 'y');
    gsap.set(proxy, { x: px + driftSpeedX * dt, y: py + driftSpeedY * dt });
    syncTrack();
  });
