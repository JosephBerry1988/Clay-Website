...
(function () {
  ...
  /* Below this viewport width the mobile board applies. */
  var MOBILE_MAX = 767;

  /* Discrete zoom stops rather than a continuous range. */
  var ZOOM_LEVELS = [1, 2, 3.2];
  ...
  ClayMap.prototype.hydrateFromSource = function () {
    ...
    var rows = source.querySelectorAll(".cmap-src");
  ...
  ClayMap.prototype.fitFluid = function (w, m) {
    ...
    var panelY = m.panel.y;
    var searchTop = panelY + panelH + m.gapAboveSearch;

    var card = this.root.querySelector(".cmap__search");
    var cardH = card ? card.offsetHeight : 336;
    var stageH = searchTop + cardH;

    var st = this.root.style;
    st.setProperty("--cmap-stage-w", w + "px");
    st.setProperty("--cmap-stage-h", stageH + "px");
    st.setProperty("--cmap-panel-w", panelW + "px");
    st.setProperty("--cmap-panel-h", panelH + "px");
    st.setProperty("--cmap-search-top", searchTop + "px");
    st.setProperty("--cmap-search-w", panelW + "px");
    st.setProperty("--cmap-world-x", this.WB.x + "px");
    st.setProperty("--cmap-world-y", this.WB.y + "px");
    st.setProperty("--cmap-world-w", this.WB.w + "px");
    st.setProperty("--cmap-world-h", this.WB.h + "px");
    ...
    this.scaler.style.height = stageH + "px";

    /* Panel size changed, so pan limits and every dot need recomputing. */
    if (this.items) {
      this.clampPan();
      this.placeDots();
      this.applyTransform();
